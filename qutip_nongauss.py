...
def relative_ergotropic_gap_qutip(rho,H,dA,dB):
    result= (ergotropy_qutip(rho, H) - local_ergotropy_qutip(rho,dA,dB))/ergotropy_qutip(rho,H)
...
def plot_general_ergotropy(type_of_state):
    ...
    if type_of_state == 'tms photonsub on mode 1':
        ...
        r_vec= np.array([-np.log(z)/2 for z in z_vec])
        t_vec = np.linspace(0.5,6,y_points)
        k_vec= np.array([1/np.tanh((1/(2*t))) for t in t_vec])
        x= np.pi/4
        X=z_vec
        Y=k_vec
    ...
    if type_of_state == 'tms photonsub on superposition of modes':
        z=0.5
        phi_vec=np.linspace(0,np.pi/4,x_points)
        t_vec = np.linspace(0.5,6,y_points)
        k_vec= np.array([1/np.tanh((1/(2*t))) for t in t_vec])
        x= np.pi/4
        X=phi_vec
        Y=k_vec
        #W = [[np.float64(find_ergotropic_gap_phsub(z,k)) for z in z_vec] for k in k_vec]
        W = [[np.float64(relative_ergotropic_gap_qutip(photon_subtract(create_two_mode_gaussian_state(t_vec[j], t_vec[j], z, 1/z, x, cutoff),cutoff,[np.cos(phi),np.sin(phi)]),hamiltonian,cutoff,cutoff)) for phi in phi_vec] for j in range(len(k_vec))]
        en = [[np.float64(log_negativity(photon_subtract(create_two_mode_gaussian_state(t_vec[j], t_vec[j], z,1/z, x, cutoff),cutoff,[np.cos(phi),np.sin(phi)]))) for phi in phi_vec] for j in range(len(k_vec))]

    X_grid, Y_grid =np.meshgrid(X,Y)
    fig,ax=plt.subplots(1,1,figsize=(10,6))
    c1=ax.pcolormesh(X_grid,Y_grid,W,cmap=cm.get_cmap('viridis_r', 15))
